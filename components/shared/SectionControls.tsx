...
export const SectionControls: React.FC<SectionControlsProps> = ({ onEdit, onMoveUp, onMoveDown, onDelete, isFirst, isLast, isHidden }) => {
    return (
        <div className={`absolute top-4 right-4 z-30 flex items-center gap-1 p-1 bg-gray-900/80 backdrop-blur-sm text-white rounded-full shadow-lg border border-white/20 ${isHidden ? 'bg-red-900/80' : ''}`}>
            <button onClick={onEdit} title="Editar Seção" className="p-2 hover:bg-gray-700 rounded-full transition-colors">
